    ...
    <nav class="bg-gray-900/95 backdrop-blur-sm border-b border-purple-900/50 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <a href="{{ route('home') }}" class="text-xl font-black bg-gradient-to-l from-pink-500 to-purple-600 bg-clip-text text-transparent flex-shrink-0">
                ...
                <div class="flex items-center gap-3">
                    @auth
                        @if(auth()->user()->is_admin)
                            <a href="{{ route('admin.dashboard') }}" class="hidden sm:block text-sm bg-purple-700 hover:bg-purple-600 text-white px-3 py-1.5 rounded-lg transition-colors">لوحة التحكم</a>
                        @endif
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center gap-2 text-gray-300 hover:text-white">
                                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-sm font-bold flex-shrink-0">
                                    {{ mb_substr(auth()->user()->name, 0, 1) }}
                                </div>
                                <span class="hidden sm:block text-sm max-w-24 truncate">{{ auth()->user()->name }}</span>
                            </button>
                    ...
                    @else
                        <a href="{{ route('login') }}" class="text-gray-300 hover:text-white text-sm hidden sm:block">تسجيل الدخول</a>
    ...
    @foreach(['success' => 'bg-green-600', 'error' => 'bg-red-600', 'warning' => 'bg-yellow-600', 'info' => 'bg-blue-600'] as $type => $color)
        @if(session($type))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" x-cloak
                class="fixed top-20 right-1/2 translate-x-1/2 z-[60] {{ $color }} text-white px-6 py-3 rounded-xl shadow-2xl max-w-md w-[90%] text-sm text-center">
                {{ session($type) }}
            </div>
        @endif
    @endforeach
