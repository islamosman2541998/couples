<x-admin-layout title="إضافة سؤال اعرفني">

    <form method="POST" action="{{ route('admin.know-me.store') }}" class="max-w-2xl space-y-6" x-data="{ answerType: 'open', choices: ['', '', '', ''] }">
        @csrf
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">
                    ...
                    <span class="text-gray-500 font-normal text-xs mr-1">يُصاغ عن الشخص الذي يُسأل عنه</span>
                </label>
                <textarea name="question" rows="2" required
                          ...
                          placeholder="مثال: إيه هو طبقه المفضل؟ / فين يحب يسافر؟">{{ old('question') }}</textarea>
                    ...
                    @foreach($categories as $key => $cat)
                        <label class="cursor-pointer">
                            <input type="radio" name="category" value="{{ $key }}"
                                   {{ old('category', 'daily') === $key ? 'checked' : '' }}
                                   class="peer sr-only">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <label class="cursor-pointer">
                        <input type="radio" name="answer_type" value="open" x-model="answerType"
                               {{ old('answer_type', 'open') === 'open' ? 'checked' : '' }}
                               class="peer sr-only">
                    ...
                    <label class="cursor-pointer">
                        <input type="radio" name="answer_type" value="choice" x-model="answerType"
                               {{ old('answer_type') === 'choice' ? 'checked' : '' }}
                               class="peer sr-only">
                        <div class="peer-checked:ring-2 peer-checked:ring-purple-500 bg-gray-800 border border-gray-700 rounded-xl p-3 flex items-center gap-3 transition-all">
            ...
            <!-- Choices (show only when choice type) -->
            ...
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">
                    تلميح للإجابة
                    <span class="text-gray-500 font-normal text-xs mr-1">اختياري — يساعد في الإجابة</span>
                </label>
                <input type="text" name="hint" value="{{ old('hint') }}"
                       ...
                       placeholder="مثال: فكّر في آخر مرة طلبتم أكل برّا">
            </div>
