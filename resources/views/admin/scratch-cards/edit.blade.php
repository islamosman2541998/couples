<x-admin-layout title="تعديل كارت سكراتش">

    <form method="POST" action="{{ route('admin.scratch-cards.update', $scratchCard) }}" enctype="multipart/form-data"
          class="max-w-2xl space-y-6">
        @csrf @method('PUT')
        <x-scratch-level-select :value="old('level', $scratchCard->level)" />
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="number" value="{{ old('number', $scratchCard->number) }}" min="1"
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $scratchCard->sort_order) }}" min="0"
                           class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-purple-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="content" rows="3" required
                          class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-purple-500 resize-none">{{ old('content', $scratchCard->content) }}</textarea>
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">اتركه فارغاً للإبقاء على الصورة الحالية</p>
                @error('image')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_active" value="1" id="is_active"
                       {{ old('is_active', $scratchCard->is_active) ? 'checked' : '' }}
