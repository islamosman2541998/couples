    ...
    <div class="max-w-2xl mx-auto px-4 py-8" x-data="spinnerGame()" x-init="init()">
        ...
        <div class="relative mx-auto mb-8" style="width: 320px; height: 320px;">
            ...
            <div class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-2 z-20">
                <div class="w-0 h-0 border-l-[12px] border-r-[12px] border-b-[24px] border-l-transparent border-r-transparent border-b-white drop-shadow-lg"></div>
            </div>
        </div>
        ...
        <div x-show="result" x-cloak
            ...
            @if(count($images) > 0)
                <img x-bind:src="result?.image" x-show="result?.image" alt="" class="w-32 h-32 object-cover rounded-2xl mx-auto mt-4 border-4 border-white/20">
            @endif
        </div>
        ...
        function spinnerGame() {
            return {
                ...
                spin() {
                    ...
                    const targetOffset = arc * winnerIdx + arc / 2;
                    const targetAngle  = this.currentAngle + extraSpins + (2 * Math.PI - (this.currentAngle % (2 * Math.PI))) - targetOffset;
